...
def index(ctx: click.Context, full: bool, path: str | None, changed_only: bool) -> None:
    ...
    click.echo(f"  {CHECK} {success('Indexing complete.')}")
...
async def _run_index(config, project_dir: str, full: bool = False, verbose: bool = False) -> None:
    ...
    parts = [f"  {CHECK} Indexed {value(str(result.total_chunks))} chunks from {value(str(len(result.indexed_files)))} files"]
    if result.deleted_files:
        parts.append(f", pruned {warn(str(len(result.deleted_files)))} deleted")
    if result.skipped_files:
        parts.append(f", skipped {dim(str(len(result.skipped_files)))} non-text")
