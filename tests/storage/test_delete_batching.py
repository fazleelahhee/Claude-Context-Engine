...
Pre-fix, vector/fts/graph stores built one IN(...) clause with len(file_paths)
placeholders. SQLite's SQLITE_MAX_VARIABLE_NUMBER (999 on older builds, 32766
on modern) means a project-wide prune touching >999 files raised
`OperationalError: too many SQL variables`.

Each test below ingests a few rows then asks the store to delete by a list
of >1500 file_paths to force multiple batches through the chunked helper.
"""
...
def test_fts_store_delete_handles_oversize_path_list(tmp_path):
    ...
    # FTS hit count == 0 after deletion.
...
def test_graph_store_delete_handles_oversize_path_list(tmp_path):
    ...
    # No nodes left for the deleted file path.
